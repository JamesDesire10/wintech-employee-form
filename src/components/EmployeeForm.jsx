...
const EmployeeForm = () => {
  ...
  const validateStep = () => {
    ...
    if (currentStep === 4) {
      ...
      if (formData.accountnum.length !== 10) return 'Account number must be exactly 10 digits.'
    }
    return null
  }
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <div key={step.id} className={`sidebar-step ${state}`}>
                {i > 0 && <div className={`sidebar-line ${i <= currentStep ? 'filled' : ''}`} />}
                <div className="sidebar-circle">
                  {i < currentStep ? '✓' : i + 1}
                </div>
                <div className="sidebar-step-text">
                  <span className="sidebar-step-label">{step.label}</span>
                  {i === currentStep && <span className="sidebar-step-status">In progress</span>}
                  {i < currentStep && <span className="sidebar-step-status done-text">Complete</span>}
                </div>
              </div>
            )
          })}
      ...
      <main className="form-main">
        <div className="form-main-inner">
          ...
          <div className="step-header">
            ...
            <h1 className="step-title">{STEPS[currentStep].icon} {STEPS[currentStep].label}</h1>
          ...
          {currentStep === 0 && (
            <div className="step-content">
              <div className="field">
                <label>01 — Full Name <span className="req">*</span></label>
          ...
          {currentStep === 1 && (
            <div className="step-content">
              <div className="field">
                ...
                <textarea name="emergency" value={formData.emergency} onChange={handleChange} placeholder="Type here" rows={4} />
              </div>
              <div className="field">
                <label>09 — Next of Kin <span className="req">*</span></label>
                <p className="hint">(Name, Relationship, Phone number and Email ID)</p>
                <textarea name="nextofkin" value={formData.nextofkin} onChange={handleChange} placeholder="Type here" rows={4} />
              </div>
              <div className="field">
          ...
          {currentStep === 3 && (
            <div className="step-content">
              <p className="hint" style={{ marginBottom: '1.5rem' }}>📄 Accepted: PDF, JPG, PNG | Max size: 10MB per file</p>
              {[
                ...
                <div className="field" key={key}>
                  ...
                  <div className="file-upload-area">
                    <div className="drop-zone">
                      ...
                      {fileNames[key] ? <span className="file-chosen">{fileNames[key]}</span> : 'Drag or paste file here'}
                    </div>
          ...
          {currentStep === 4 && (
            <div className="step-content">
              <div className="field">
                ...
                <p className="hint">Input your bank name and account name (e.g. Access Bank — James Akonuche Desire)</p>
            ...
            {currentStep < STEPS.length - 1 ? (
              ...
              >
                {status === 'uploading' ? '⏳ Uploading files…' :
                 status === 'saving' ? '⏳ Saving data…' : '✓ Submit Form'}
              </button>
            )}
          </div>
